...
bucket 998 전용 배포 단위다.  오디오 10.0초 (998 frame) 입력을
192차원 화자 임베딩으로 바꾼다.
...
| 파일 | 내용 |
|---|---|
| `campp_sv_998.camppmodel` | execution plan, packed weights, streaming weights/schedule, kernel dispatch table, frontend/postprocess 계약 |
...
- tensor arena 배치
- layer-hybrid kernel dispatch 표: {"fused_combined_fixed": 6, "fused_combined_v5": 55, "qconv_mac_fixed": 2, "qconv_v5": 111}
...
./campp_runtime --plan <plan.bin> --weights <weights.bin> \
    --input <feature.f32> --audio-seconds 10.0 \
...
입력은 FBank 998x80 float32다.  파라미터는 모델의 frontend 계약을 따른다
