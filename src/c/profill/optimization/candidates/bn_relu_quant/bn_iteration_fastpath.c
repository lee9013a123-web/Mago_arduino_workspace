...
static uint64_t campp_bn_max_offset(
    ...
    CamppBnIterationPlan *out_plan)
{
    ...
    if (campp_bn_max_offset(dimensions, out_plan->input_strides) +
            sizeof(float) > input->storage_span_bytes ||
        campp_bn_max_offset(dimensions, out_plan->output_strides) + 1u >
            output->storage_span_bytes) {
        return false;
    }
    out_plan->channel_contiguous =
        out_plan->input_strides[1] == sizeof(float) &&
        out_plan->output_strides[1] == 1u;
    return true;
}
