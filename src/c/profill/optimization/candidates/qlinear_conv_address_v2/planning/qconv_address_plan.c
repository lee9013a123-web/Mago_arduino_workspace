...
#include "../kernels/qconv_address_internal.h"

static bool campp_qconv_address_v2_interior_range(
    ...
    CamppQconvAddressPlan *out_plan)
{
    ...
    for (axis = 0u; axis < geometry->spatial_rank; ++axis) {
        if (geometry->input_spatial[axis] == 0u ||
            ...
            !campp_qconv_address_v2_multiply_u64(
                ...
                &kernel_elements) ||
            !campp_qconv_address_v2_interior_range(
    ...
    if (output_spatial_count > UINT32_MAX ||
        kernel_elements > CAMPP_QCONV_ADDRESS_KERNEL_MAX) {
        return CAMPP_QCONV_ADDRESS_UNSUPPORTED;
    }
    ...
    if (geometry->spatial_rank == 1u && geometry->kernel_shape[0] == 1u &&
        ...
        out_plan->preferred_path = CAMPP_QCONV_ADDRESS_PATH_ONE_BY_ONE;
        ...
        out_plan->preferred_path = CAMPP_QCONV_ADDRESS_PATH_GENERIC;
    }
