#ifndef CAMPP_PROFILL_QCONV_V5_ADDRESS_INTERNAL_H
#define CAMPP_PROFILL_QCONV_V5_ADDRESS_INTERNAL_H
...
static inline bool campp_qconv_v5_address_add_u64(
...
static inline bool campp_qconv_v5_address_multiply_u64(
    uint64_t left, uint64_t right, uint64_t *out_value)
{
...
static inline bool campp_qconv_v5_address_coordinate(
...
static inline bool campp_qconv_v5_address_group_base(
    ...
    uint32_t group_channel, uint64_t *out_offset)
{
    ...
    return campp_qconv_v5_address_add_u64(
        batch_offset, channel_offset, out_offset);
}

static inline bool campp_qconv_v5_address_add_spatial_offset(
    const CamppQconvV5AddressPlan *plan, uint8_t axis,
...
static inline bool campp_qconv_v5_address_point_in_storage(
    const CamppQconvV5AddressPlan *plan, uint64_t offset)
{
    return plan != NULL && offset <= plan->geometry.storage_span_bytes &&
        plan->geometry.input_channels_per_group <=
            plan->geometry.storage_span_bytes - offset;
}

static inline const uint8_t *campp_qconv_v5_address_pointer(
    const CamppQconvV5AddressPlan *plan, uint64_t offset)
{
    return plan->geometry.input_base + (size_t)offset;
}

#endif /* CAMPP_PROFILL_QCONV_V5_ADDRESS_INTERNAL_H */
