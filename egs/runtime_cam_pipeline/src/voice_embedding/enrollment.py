"""Five-utterance speaker enrollment and template aggregation."""
...
DEFAULT_RECORDINGS = 5


def validate_speaker_folder(value: str) -> str:
    name = value.strip()
    if not name or name in {".", ".."}:
        raise RuntimePipelineError("speaker folder name is empty or unsafe")
    if any(character in name for character in ("/", "\\", "\0")):
        raise RuntimePipelineError(
            "speaker folder name must not contain path separators"
        )
    if any(ord(character) < 32 for character in name):
        raise RuntimePipelineError("speaker folder name contains control characters")
    return name
...
def l2_normalize(vector: np.ndarray) -> np.ndarray:
    ...
    if not np.isfinite(norm) or norm <= 1e-12:
        raise RuntimePipelineError("cannot normalize a zero or non-finite embedding")
    return (value / np.float32(norm)).astype(np.float32)


def aggregate_embeddings(embeddings: list[np.ndarray]) -> np.ndarray:
    if not embeddings:
        raise RuntimePipelineError("no enrollment embeddings were produced")
    normalized = np.stack([l2_normalize(item) for item in embeddings])
    return l2_normalize(normalized.mean(axis=0, dtype=np.float32))
...
def enroll_speaker(
    *, repo_root: Path, pipeline_root: Path, profile: MicrophoneProfile,
    ...
    if recording_count <= 0:
        raise RuntimePipelineError("recording count must be positive")
    if countdown_seconds < 0:
        raise RuntimePipelineError("countdown must not be negative")
    recorded_root = pipeline_root / "voice/recorded" / speaker
    embedded_root = pipeline_root / "voice/embedded" / speaker
    run_root = pipeline_root / "runs/enrollment" / speaker
    ...
    if not force and (template_path.exists() or metadata_path.exists()):
        raise RuntimePipelineError(
            f"speaker enrollment already exists: {embedded_root}; use --force"
        )
    ...
    for index in range(1, recording_count + 1):
        ...
        output(f"  completed: {wav_path.name}")
    ...
    metadata = {
        ...
        "microphone_device": profile.device,
